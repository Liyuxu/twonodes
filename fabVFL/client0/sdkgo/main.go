...
func main() {
	...
	contract := network.GetContract("fabVFL")
	...
	submitEmbeddingTimeSlice := make([]string, numIter)
	...
	for iter := 0; iter < numIter; iter++ {
		...
		subembeddcost := time.Since(subembeddstart).Microseconds()
		submitEmbeddingTimeSlice[iter] = strconv.Itoa(int(subembeddcost))
		...
		subcontricost := time.Since(subcontristart).Microseconds()
		submitTimeSlice[iter] = strconv.Itoa(int(subcontricost))
		fmt.Println(contributionContractName + " SubmitContribution success." + string(result))
	}

	File, err := os.OpenFile("Client0B"+strconv.Itoa(batch)+"submitContriTime.csv", os.O_RDWR|os.O_APPEND|os.O_CREATE, 0666)
	if err != nil {
		log.Println("文件打开失败！")
	}
	defer File.Close()
	...
	log.Println("Contribution数据写入成功...")

	File2, err := os.OpenFile("Client0B"+strconv.Itoa(batch)+"submitEmbeddingTime.csv", os.O_RDWR|os.O_APPEND|os.O_CREATE, 0666)
	if err != nil {
		log.Println("文件打开失败！")
	}
	defer File2.Close()
	...
	//创建写入接口
	WriterCsv2 := csv.NewWriter(File2)

	//写入一条数据，传入数据为切片(追加模式)
	err = WriterCsv2.Write(submitEmbeddingTimeSlice)
	if err != nil {
		log.Println("WriterCsv写入文件失败")
	}
	WriterCsv2.Flush() //刷新，不刷新是无法写入的
	log.Println("Embedding数据写入成功...")
