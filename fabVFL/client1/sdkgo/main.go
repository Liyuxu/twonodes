...
func main() {
	...
	// FL
	numNodes, err := strconv.Atoi(taskInfo.NumNodes)
	if err != nil {
		fmt.Println("numNodes, err := strconv.Atoi(taskInfo.NumNodes) fail.")
	}
	...
	for iter := 0; iter < numIter; iter++ {
		...
		if err != nil {
			fmt.Println("lens, err := connection.Read(buf) fail.")
		}
		signal := buf[:lens]
		//解析信号
